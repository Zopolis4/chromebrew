...
class PackageUtils
  ...
  def self.get_binary_url(pkg)
    ...
    return "https://gitlab.com/api/v4/projects/26210301/packages/generic/#{pkg.name}/#{pkg.version}_#{ARCH}/#{pkg.name}-#{pkg.version}-chromeos-#{ARCH}.#{pkg.binary_compression}" if package_id == 0
